...
`, () => {
  expect(advancedSettingsSql(getBoostedSettingsDefaults('BOOSTED_TREE_REGRESSOR'))).toEqual(
    ", BOOSTER_TYPE = 'GBTREE' , NUM_PARALLEL_TREE = 1 , DART_NORMALIZE_TYPE = 'TREE' , TREE_METHOD = 'AUTO' , MIN_TREE_CHILD_WEIGHT = 1 , COLSAMPLE_BYTREE = 1 , COLSAMPLE_BYLEVEL = 1 , COLSAMPLE_BYNODE = 1 , MIN_SPLIT_LOSS = 0 , MAX_TREE_DEPTH = 6 , SUBSAMPLE = 1 , L1_REG = 0 , L2_REG = 0 , EARLY_STOP = true , LEARN_RATE = 0.3 , MAX_ITERATIONS = 20 , MIN_REL_PROGRESS = 0.01 , DATA_SPLIT_METHOD = 'AUTO_SPLIT' , ENABLE_GLOBAL_EXPLAIN = false "
  )
})
